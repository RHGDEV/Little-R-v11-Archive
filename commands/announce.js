...
function sendAnnounce(bot, message, args, guild) {
  var sendChannel = "";
  if (config.default.donotAnnounce.includes(guild.id)) return message.channel.send(`🗳 Faild to send announcement to ${guild.name} (In DNA list)`).then(m => m.delete(20000));
  ...
  let announce = new discord.RichEmbed()
    ...
    .setFooter(`An official announcement from Rusty. (RHG#0822)`, bot.user.avatarURL)
    .setTimestamp();
  ...
  if (sendChannel !== "") {
    sendChannel.send({ embed: announce })
      ...
      .catch(err => {
        message.channel.send(`🗳 Faild to send announcement to ${guild.name} (Send Error)`).then(m => m.delete(20000));
      });
  } else {
    message.channel.send(`🗳 Faild to send announcement to ${guild.name} (No channel)`).then(m => m.delete(20000));
  }
}
