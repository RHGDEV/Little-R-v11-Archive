...
premiumServers = config.premiumServers,
...
YTapi = new YouTube(process.env.ytapikey ? process.env.ytapikey : config.keys.ytapikey);

// npm i ffmpeg ffmpeg-binaries get-youtube-id opusscript simple-youtube-api youtube-info youtube-node ytdl-core
// github:fent/node-ytdl-core#master
...
function play(connect, msg, bot) {
  ...
  if (server.queueLength[0] !== "0:0:0") {
    ...
    //server.dispatcher = connect.playStream(YTDL(server.queueList[0], { filter: "audioonly" }));
...
module.exports = (bot, db, message) => {
  ...
  //return message.channel.send(`I'm sorry looks like you wanted to play some music i'm I right?\nBut, my music module is currently broken/borked and as of now I can't find no fix...`);
  ...
  switch (args[0].toLowerCase()) {
    case "help":
      ...
      let em = new Discord.RichEmbed()
        ...
        .addField(`${prefix}fskip`, `Force skips the current song.`)
        .addField(`${prefix}skip`, `Skips the current song.`)
        .addField(`${prefix}stop`, `Stops everything.`)
